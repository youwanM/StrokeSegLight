...
class nnUNetExporter:
    def __init__(self, root):
        self.root = root
        self.root.title("nnU-Net to ONNX Converter")
    ...
    def create_widgets(self):
        ...
        # 5. Quantization Selection (Dropdown List)
        tk.Label(self.root, text="5. Select Quantization Method:", font=("Arial", 10, "bold")).pack(anchor="w", **pad)
    ...
    def browse_pth(self):
        ...
        if file:
            ...
            # Auto-detect plans.json and dataset.json based on standard nnU-Net folder structure
            try:
                # Structure: DatasetXXX / Trainer__Plans__Config / fold_X / checkpoint.pth
                ...
                pass # If folder structure is non-standard, just leave the fields empty
    ...
    def get_nnunet_model(self, pth_file, plans_file, dataset_file):
        ...
        network = get_network_from_plans(
            ...
            num_output_channels,
            allow_init=True,
            deep_supervision=False # Critical: must be False for ONNX
        )
