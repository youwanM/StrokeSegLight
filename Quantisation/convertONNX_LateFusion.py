...
class LateFusionNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        out_flair = self.unet_flair(x_flair)
        
        if isinstance(out_t1, (list, tuple)):
            res = []
            for ds_t1, ds_flair in zip(out_t1, out_flair):
                combined_logits = torch.cat([ds_t1, ds_flair], dim=1)
                res.append(self.fusion_conv(combined_logits))
            return res
        else:
            combined_logits = torch.cat([out_t1, out_flair], dim=1)
